...
public class SpaceshipRepairModule : Town {
    public SpaceshipRepairModule(
        Planet planet,
        int level
    )
    {
    ...
    public bool RepairSpaceship()
    {
        Recources.Resources repairResources = Manager.CreateResources(10);
        if (!Planet.IsEnoughResources(repairResources)) return false;
        Planet.UpdateAvailableResources(repairResources);
        return true;
    }
}